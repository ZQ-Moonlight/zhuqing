...
import { getContentRoot, walkContentFiles } from "../../../../lib/contentArchive";
...
export const getStaticPaths: GetStaticPaths = async () => {
    ...
    const files = await walkContentFiles(nodePath.join(root, "markdown-img"));

    return files
        .map((file) => toPosix(nodePath.relative(nodePath.join(root, "markdown-img"), file)))
...
export const GET: APIRoute = async ({ params }) => {
    const root = nodePath.resolve(getContentRoot(), "markdown-img");
